...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            # Dynamically reshape emb_out to match h's spatial dimensions
            spatial_shape = h.shape[2:]  # Get the spatial dimensions of h (e.g., [7, 7] or [14, 14])
            emb_out = emb_out.squeeze(1).squeeze(1).squeeze(1)  # Remove singleton dimensions
            emb_out = emb_out[:, :, None, None].expand(-1, -1, *spatial_shape)  # Expand to match spatial dimensions
            h = h + emb_out
            h = self.out_layers(h)
